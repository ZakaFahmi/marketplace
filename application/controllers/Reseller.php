...
class Reseller extends CI_Controller {
	...
	function proses_penjualan(){
		...
		if ($this->input->post('id')!=''){
			$this->load->view('reseller/mod_penjualan/view_modal_status');
		}else{
			if (isset($_POST['resi'])){
				$data1 = array('proses'=>'1','resi'=>$this->input->post('resi'));
				$where1 = array('id_penjualan'=>$this->input->get('id'),'id_penjual'=>$this->session->id_reseller);
				$this->model_app->update('rb_penjualan', $data1, $where1);
			}else{
				$data = array('proses'=>$this->input->get('status'));
				$where = array('id_penjualan' =>$this->input->get('id'));
				$this->model_app->update('rb_penjualan', $data, $where);
			}
